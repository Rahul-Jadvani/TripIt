...
def fetch_user_stats(user_id):
    ...
    try:
        ...
        ).outerjoin(Project, and_(Project.user_id == user_id, Project.is_deleted == False))\
        ...
        return {
            ...
            'badges_awarded': stats.badges_awarded or 0,
...
def fetch_my_projects(user_id):
    ...
    try:
        projects = Project.query.filter_by(user_id=user_id, is_deleted=False)\
            .options(joinedload(Project.creator))\
            .order_by(Project.created_at.desc())\
            .limit(10).all()

        return [p.to_dict(include_creator=True, user_id=user_id) for p in projects]
    except Exception as e:
