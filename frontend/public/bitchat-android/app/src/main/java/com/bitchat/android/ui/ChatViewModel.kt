    ...
    fun sendSOSMessage() {
        viewModelScope.launch {
            try {
                ...
                // Get the last known location using the public method
                val location = locationManager.getLastKnownLocation()

                if (location != null) {
                    val latitude = location.latitude
                    val longitude = location.longitude
                    val sosMessage = "SOS $latitude,$longitude"

                    Log.d(TAG, "Sending SOS broadcast: $sosMessage")

                    // Broadcast to all peers on mesh network
                    ...
                    Log.w(TAG, "Cannot send SOS: Location not available")
